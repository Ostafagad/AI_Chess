...
class AI:
    ...
    def evaluate(self, gametiles):
        ...
        movex = move()
        tp.clear()
        xp = self.minimax(gametiles, input_depth, -
                ...
                for x in range(8):
                    for y in range(8):
                        if gametiles[y][x].pieceonTile.alliance == 'Black':
                            moves1 = gametiles[y][x].pieceonTile.legalmoveb(
                                gametiles)
        ...
        if not player:
            ...
            for lk in kp:
                for move in lk:
                    ...
                    gametiles = self.move(
                        gametiles, move[0], move[1], move[2], move[3])
                    evalk = self.minimax(gametiles, depth-1, alpha, beta, True)
                    if evalk < minEval and depth == input_depth:
                        tp.clear()
                        tp.append(move)
                    if evalk == minEval and depth == input_depth:
                        tp.append(move)
                    ...
                    beta = min(beta, evalk)
                    gametiles = self.revmove(
                        gametiles, move[2], move[3], move[0], move[1], mts)
                    if beta <= alpha:
                        break
            ...
            for lk in ks:
                for move in lk:
                    mts = gametiles[move[2]][move[3]].pieceonTile
                    gametiles = self.movew(
                        gametiles, move[0], move[1], move[2], move[3])
                    evalk = self.minimax(
                        gametiles, depth-1, alpha, beta, False)
    ...
    def calci(self, gametiles, y, x, moves):
        ...
        for move in moves:
            ...
            gametiles[move[0]][move[1]
                               ].pieceonTile = gametiles[y][x].pieceonTile
            gametiles[y][x].pieceonTile = nullpiece()
            mk = self.calculateb(gametiles)
            gametiles[y][x].pieceonTile = gametiles[move[0]
                                                    ][move[1]].pieceonTile
    ...
    def move(self, gametiles, y, x, n, m):
        ...
        if promotion == True:
            ...
            if gametiles[y][x].pieceonTile.tostring() == 'P':
                gametiles[y][x].pieceonTile = nullpiece()
                gametiles[n][m].pieceonTile = queen(
                    'Black', self.updateposition(n, m))
                promotion = False
